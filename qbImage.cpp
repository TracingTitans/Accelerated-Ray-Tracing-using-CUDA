...
void qbImage::Display(bool flipImage)
{
	...
	if (flipImage == true)
	{
		SDL_RenderCopyEx(m_pRenderer, m_pTexture, &srcRect, &bounds, 0.0, NULL, SDL_FLIP_VERTICAL);
	}
	else
	{
		SDL_RenderCopyEx(m_pRenderer, m_pTexture, &srcRect, &bounds, 0.0, NULL, SDL_FLIP_NONE);
	}
